...
import { getOrdersCount } from "../../helpers/utils"
...
const Cart = styled.button`
  ...
  &:disabled {
    cursor: default;
    &:hover path { fill: var(--color-white); }
  }
`
...
const CartBtn: React.FC<ICartBtn> = ({ click, modal }) => {
  const { orders } = useAppStore()
  const count = getOrdersCount(orders)

  return (
    <Cart onClick={() => click(!modal)} disabled={!orders.length}>
      ...
      {count > 0 && <CartCounter>{count}</CartCounter>}
    </Cart>
  )
}
