...
const CartBtn = styled.button`
  ...
  &:hover path { fill: var(--color-primary); }
`
const CartDropdown = styled.div<{ $open: boolean }>`
  ...
  z-index: 500;
`
const CartOverlay = styled.div<{ $open: boolean }>`
...
const Cart: React.FC = () => {
  ...
  return (
    <>
      <CartBtn onClick={() => setModal(!modal)}>
      ...
      <CartDropdown $open={modal}>
        cart (3)
        Remove all
        <Btn value="Checkout" expand />
      </CartDropdown>
      <CartOverlay $open={modal} onClick={() => setModal(false)} />
    </>
  )
}
